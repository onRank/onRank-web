...
function TimeSelector({ value, onChange, disabled = false }) {
  ...
  const [selectedHours, selectedMinutes] = value ? value.split(':').map(Number) : [0, 0];
  ...
  const handleMinuteSelect = (minute) => {
    const newValue = `${String(selectedHours).padStart(2, '0')}:${String(minute).padStart(2, '0')}`;
    onChange(newValue);
  };
    ...
    <div style={{ position: 'relative', width: '100%' }} ref={dropdownRef}>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: '4px',
          fontSize: '16px',
          backgroundColor: disabled ? colors.hoverBackground : colors.inputBackground,
      ...
      >
        <span>
          {String(displayHours).padStart(2, '0')}:{String(selectedMinutes).padStart(2, '0')} {isPM ? 'PM' : 'AM'}
        </span>
        <span>▼</span>
      </button>
            ...
            {minutes.map(minute => (
              <div
                ...
                style={{
                  padding: '8px 12px',
                  cursor: 'pointer',
                  backgroundColor: minute === selectedMinutes 
                    ? colors.hoverBackground 
                    : 'transparent',
                  textAlign: 'center'
                }}
