  ...
  useEffect(() => {
    const fetchAttendanceData = async () => {
      try {
        ...
        // 출석 정보 가져오기
        const attendanceData = await studyService.getAttendances(studyId);
        setAttendances(attendanceData);

        // 출석 통계 계산
        if (attendanceData && attendanceData.length > 0) {
          const stats = attendanceData.reduce((acc, curr) => {
            if (curr.attendanceStatus === 'PRESENT') acc.present += 1;
            else if (curr.attendanceStatus === 'ABSENT') acc.absent += 1;
            else if (curr.attendanceStatus === 'LATE') acc.late += 1;
            else acc.unknown += 1;
            return acc;
