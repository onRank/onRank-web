...
const notificationService = {
  ...
  getNotifications: async () => {
    try {
      ...
      const config = {
        ...
        referrerPolicy: 'no-referrer' // Referrer-Policy 설정 추가
      };
  ...
  markAsRead: async (notificationId) => {
    try {
      ...
      // 백엔드 API 명세에 맞게 PATCH 요청 설정 - Referrer-Policy 설정 추가
      const response = await api.patch(`/notifications/${notificationId}/read`, {}, {
        ...
        timeout: 10000, // 타임아웃 연장
        referrerPolicy: 'no-referrer' // Referrer 정보를 전혀 보내지 않음
      });
      ...
      if (error.message && error.message.includes('Network Error')) {
        ...
        // 최대 1회 재시도
        try {
          console.log('[notificationService] 알림 읽음 처리 재시도 중... (Referrer 설정 변경)');
          const retryResponse = await api.patch(`/notifications/${notificationId}/read`, {}, {
            ...
            referrerPolicy: 'origin' // 두 번째 시도는 다른 정책으로 시도
          });
