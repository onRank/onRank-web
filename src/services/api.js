        ...
        try {
          for (const file of files) {
            ...
            if (
              ...
              console.log(
                `[noticeService] 파일 업로드 성공: ${file.originalName}`
              );
            } else {
              console.error(
                `[noticeService] 파일 업로드 실패: ${file.originalName}`
              );
              setUploadMessage("[noticeService] 일부 파일 업로드 실패");
            }
          }
          setUploadMessage("[noticeService] 파일 업로드 성공");
        } catch (uploadError) {
